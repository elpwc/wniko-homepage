...
export default function Contact(props: P) {
  ...
  const [avatarRotate, setAvatarRotate]: [number, any] = useState(0);
    ...
    <div style={{ position: 'absolute', zIndex: 0, paddingTop: '30px', height: '100%', width: '100%' }}>
      <div className='white-card' style={{ position: 'absolute', top: '-30px', paddingLeft: '300px', paddingTop: '30px', height: '220px' }}>
        <div style={{ position: 'absolute', bottom: '0px' }}>
          <Space size='large'>
            <h1>Wniko <span style={{ color: 'rgb(100, 100, 100)' }}><i>"Elpwc"</i></span> Wang</h1>
            <h3>性别: ?</h3>
            <h3>居住地：淫梦大公国</h3>
            <h3></h3>
          </Space>
      ...
      <div className='white-card' style={{ height: '200px', marginTop: '220px', padding: '30px 10px' }}>
        <div style={{ backgroundColor: 'plum', fontSize: '15px', position: 'absolute', color: 'white', left: '0px', width: '200px', textAlign: 'right', paddingRight: '20px' }}><p>使用的技术</p></div>
        <div style={{ marginTop: '50px' }}>
          {imgImports.map((img) => {
            // @ts-ignore
            return <a target='_blank' rel="noreferrer" href={img[3]} key={img[0]}><Tooltip title={img[2]}><img alt={img[0].split('.')[0]} src={img[1].default} width='50px' /></Tooltip></a>
          })}
        </div>
      ...
      <div className='white-card' style={{ height: 'fit-content', marginTop: '20px', padding: '30px 50px' }}>
        <div style={{ backgroundColor: 'plum', fontSize: '15px', position: 'absolute', color: 'white', left: '0px', width: '200px', textAlign: 'right', paddingRight: '20px' }}><p>取得联系</p></div>
        <div style={{ marginTop: '50px' }}>
          <List
